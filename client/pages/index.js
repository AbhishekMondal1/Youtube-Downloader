...
export default function Home() {
  ...
  const fetchData = async () => {
    const res = await axios.get(`/api/videodetails?id=${url}`);
    if (res.data.validid == "false") {
      setInputError(true);
    } else {
      if (res.data) {
        setUrlID(res.data.videoid);
    ...
    <>
      ...
      <div className={styles.d_flex}>
        ...
        <a
          ref={downloadLink}
          download
          href={`${process.env.NEXT_PUBLIC_VIDEO_HOST_SERVER_URL}/downloadfile/${fileurl}`}
          style={{ display: "none" }}
        >
          Download
        </a>
